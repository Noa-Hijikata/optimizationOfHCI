        ...
        for uid, g in logs.groupby("user_id"):
            ...
            act_cnt = collections.Counter(
                [
                    v
                    for v in g[
                        (g["event"] == "button") & (~g["value"].str.contains("追加"))
                    ]["value"]
                ]
            )
            base = ["登録", "下書き保存", "クリア"]
            ...
            stations = [
                v
                for v in g[g["field"].isin(["交通費_出発", "交通費_到着"])]["value"]
                if v
            ]
            vendors = [
                v
                for v in g[g["field"].isin(["店名", "宿泊名", "vendor"])]["value"]
                if v
            ]
            ...
            cfg = {
                ...
                "suggest": {
                    ...
                    "purpose_phrases": topn(phrases, 5)
                    or ["顧客訪問", "定例会議", "出張"],
                },
                "defaults": {
            ...
            with open(
                os.path.join(CONFIG_DIR, f"{uid}.json"), "w", encoding="utf-8"
            ) as f:
